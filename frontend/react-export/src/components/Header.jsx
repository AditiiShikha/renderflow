...
export default function Header({ pumpStatusColor, conveyorStatusColor, pumpCritical, onToggleSidebar }) {
  return (
    <>
      <nav className="flex items-center gap-4 px-5 py-3 flex-wrap" style={{ borderBottom: '1px solid var(--color-divider)' }}>
        ...
        <div className="flex items-center gap-2 text-[11px] tracking-wide uppercase opacity-60 px-2.5 py-1.5" style={{ border: '1px solid var(--color-divider)' }}>
          <span className="w-1.5 h-1.5 rounded-full inline-block animate-rf-blink" style={{ background: '#4f9d72' }} />
          LLM · Phi-3 · On-Prem
        </div>
        <button
      ...
      >
        <div className="flex items-center gap-2">
          <PumpSchematic statusColor={pumpStatusColor} critical={pumpCritical} />
          <span className="text-[10px] tracking-wide uppercase opacity-70">Pump 3</span>
        </div>
        <div className="flex items-center gap-2">
          <ConveyorSchematic statusColor={conveyorStatusColor} />
